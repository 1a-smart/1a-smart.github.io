import "../src/style.css";
import { useState } from "react";
import { createRoot } from "react-dom/client";

function App() {
  const [mobileMenuOpen, setMobileMenuOpen] = useState(false);
  const baseUrl = import.meta.env.BASE_URL;

  // Common classes
  const containerClass = "max-w-7xl mx-auto px-4 sm:px-6 lg:px-8";
  const navLinkClass = "text-gray-300 hover:text-cyan-400 px-3 py-2 rounded-md text-sm font-medium";
  const navLinkMobileClass =
    "text-gray-300 hover:text-cyan-400 block px-3 py-2 rounded-md text-base font-medium";
  const tableRowBase =
    "block md:table-row bg-slate-700 border-0 md:border md:border-slate-900 md:border-l-0 md:border-r-0";
  const timeCellClass = "px-6 py-0 md:py-4 text-gray-300 block md:table-cell";
  const activityCellClass = "px-6 py-0 pb-6 md:py-4 text-white block md:table-cell";
  const activityCellTopClass = "px-6 pt-0 pb-6 md:py-4 text-white block md:table-cell";
  const dayCellClass = "text-white text-2xl font-medium block md:table-cell";
  const dayCellSingleClass =
    "px-6 pt-6 pb-0 md:py-4 text-white text-2xl font-medium block md:table-cell";
  const serviceCardClass =
    "bg-slate-700 rounded-lg p-8 text-center transition-all duration-300 hover:-translate-y-1";
  const cardbackCardClass =
    "bg-white rounded-lg p-4 text-center transition-all duration-300 hover:-translate-y-1 hover:shadow-lg";
  const badgeClass = "bg-slate-800 px-4 py-2 rounded-lg";
  const footerLinkClass = "text-gray-400 hover:text-cyan-400";
  const footerHeadingClass = "text-lg font-semibold mb-4 text-cyan-400";

  // Navigation items
  const navItems = [
    { href: "#home", label: "Home" },
    { href: "#unser-angebot", label: "Unser Angebot" },
    { href: "#tcg-games", label: "Unsere TCG Auswahl" },
    { href: "#wochenplan", label: "Wochenplan" },
    { href: "#ueber-uns", label: "Über Uns" },
  ];

  return (
    <>
      <nav className="sticky top-0 w-full bg-slate-800/95 backdrop-blur-sm z-50 border-b border-slate-700">
        <div className={containerClass}>
          <div className="flex justify-between items-center h-16">
            <h1 className="text-xl font-bold text-cyan-400">1A Gamer Lounge</h1>
            <div className="hidden md:block">
              <div className="ml-10 flex items-baseline space-x-4">
                {navItems.map((item) => (
                  <a key={item.href} href={item.href} className={navLinkClass}>
                    {item.label}
                  </a>
                ))}
              </div>
            </div>
            <button
              className="md:hidden text-gray-300 hover:text-cyan-400"
              onClick={() => setMobileMenuOpen(!mobileMenuOpen)}
            >
              <svg className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path
                  strokeLinecap="round"
                  strokeLinejoin="round"
                  strokeWidth="2"
                  d="M4 6h16M4 12h16M4 18h16"
                />
              </svg>
            </button>
          </div>
        </div>
        <div
          className={`md:hidden ${mobileMenuOpen ? "" : "hidden"} bg-slate-800 border-t border-slate-700`}
        >
          <div className="px-2 pt-2 pb-3 space-y-1">
            {navItems.map((item) => (
              <a key={item.href} href={item.href} className={navLinkMobileClass}>
                {item.label}
              </a>
            ))}
          </div>
        </div>
      </nav>
      <section
        id="home"
        className="min-h-screen flex pt-32 bg-cover bg-center bg-no-repeat relative"
        style={{ backgroundImage: `url(${baseUrl}images/hero.jpg)` }}
      >
        <div className="absolute inset-0 bg-slate-900/60"></div>
        <div className={`${containerClass} relative z-10 w-full`}>
          <div className="mx-auto lg:mx-0 text-center lg:text-left max-w-3xl">
            <h1 className="text-4xl md:text-6xl font-bold mb-6">
              Play. Compete. Chill.
              <br /> Alles in der 1A Gamer Lounge.
            </h1>
            <p className="text-xl md:text-2xl text-gray-300 mb-8">
              Dein Wohnzimmer für Gaming, TCG & Turniere!
            </p>
            <div className="space-x-2">
              <a
                href="#unser-angebot"
                className="bg-cyan-500 hover:bg-cyan-600 text-white px-8 py-3 rounded-lg font-semibold"
              >
                Mehr erfahren
              </a>
              <a
                href="#ueber-uns"
                className="border border-cyan-500 text-cyan-400 hover:bg-cyan-500 hover:text-white px-8 py-3 rounded-lg font-semibold"
              >
                Über uns
              </a>
            </div>
          </div>
        </div>
      </section>

      <section id="unser-angebot" className="py-20 bg-slate-800">
        <div className={containerClass}>
          <div className="text-center mb-16">
            <h2 className="text-3xl md:text-4xl font-bold mb-4 text-cyan-400">Unser Angebot</h2>
            <p className="text-xl text-gray-300">Entdecke das perfekte Gaming-Erlebnis für dich</p>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            <div className={serviceCardClass}>
              <svg
                className="w-16 h-16 mx-auto mb-6 text-purple-400"
                viewBox="0 0 100 100"
                fill="currentColor"
              >
                <rect x="20" y="15" width="60" height="70" rx="5" fill="currentColor" />
                <rect x="25" y="20" width="50" height="30" rx="3" fill="#1e293b" />
                <rect x="30" y="55" width="40" height="3" rx="1" fill="#1e293b" />
                <rect x="30" y="62" width="25" height="3" rx="1" fill="#1e293b" />
                <rect x="30" y="69" width="35" height="3" rx="1" fill="#1e293b" />
                <circle cx="45" cy="35" r="8" fill="#8b5cf6" />
              </svg>

              <h3 className="text-2xl font-bold mb-4 text-purple-400">Trading Card Games</h3>
              <p className="text-gray-300 mb-6">
                Erlebe den Nervenkitzel von TCG-Turnieren, Deck-Building und kompetitivem Spiel mit
                anderen Enthusiasten.
              </p>
            </div>

            <div className={serviceCardClass}>
              <svg
                viewBox="0 5 32 22"
                className="w-16 h-16 mx-auto mb-6 text-cyan-400"
                fill="currentColor"
              >
                <path
                  d="m31.812 18.406-2.127-9.255A5.98 5.98 0 0 0 24 5a5.93 5.93 0 0 0-4.449 2h-7.102A5.93 5.93 0 0 0 8 5a5.97 5.97 0 0 0-5.684 4.151L.188 18.41A7 7 0 0 0 0 20c0 3.86 3.14 7 7 7a7.01 7.01 0 0 0 6.319-4h5.363a7.01 7.01 0 0 0 6.319 4c3.86 0 7-3.14 7-7a7 7 0 0 0-.189-1.594M25 26a6 6 0 0 1-5.651-4h-6.698A5.997 5.997 0 0 1 1 20c0-.471.06-.927.162-1.366l2.104-9.173C3.919 7.458 5.779 6 8 6c1.63 0 3.065.792 3.977 2h8.045C20.935 6.792 22.37 6 24 6c2.221 0 4.081 1.458 4.733 3.461l2.104 9.173c.103.439.163.895.163 1.366a6 6 0 0 1-6 6m-5-15a2 2 0 1 0-.001 3.999A2 2 0 0 0 20 11m0 3c-.551 0-1-.449-1-1s.449-1 1-1 1 .449 1 1-.449 1-1 1m5-4a2 2 0 1 0-.001 3.999A2 2 0 0 0 25 10m0 3c-.551 0-1-.449-1-1s.449-1 1-1 1 .449 1 1-.449 1-1 1m-5 3a2 2 0 1 0-.001 3.999A2 2 0 0 0 20 16m0 3c-.551 0-1-.449-1-1s.449-1 1-1 1 .449 1 1-.449 1-1 1m5-4a2 2 0 1 0-.001 3.999A2 2 0 0 0 25 15m0 3c-.551 0-1-.449-1-1s.449-1 1-1 1 .449 1 1-.449 1-1 1m-14-5h-1v-1a1 1 0 0 0-1-1H7a1 1 0 0 0-1 1v1H5a1 1 0 0 0-1 1v2a1 1 0 0 0 1 1h1v1a1 1 0 0 0 1 1h2a1 1 0 0 0 1-1v-1h1a1 1 0 0 0 1-1v-2a1 1 0 0 0-1-1m0 3H9v2H7v-2H5v-2h2v-2h2v2h2z"
                  fill="currentColor"
                />
              </svg>

              <h3 className="text-2xl font-bold mb-4 text-cyan-400">Videospiele</h3>
              <p className="text-gray-300 mb-6">
                Tauche ein in die neuesten Videospiele mit High-End Gaming-Setups und
                Multiplayer-Turnieren.
              </p>
            </div>

            <div className={serviceCardClass}>
              <svg
                className="w-16 h-16 mx-auto mb-6 text-green-400"
                viewBox="0 0 100 100"
                fill="currentColor"
              >
                <rect x="20" y="20" width="60" height="60" rx="5" fill="currentColor" />
                <rect x="25" y="25" width="50" height="50" rx="3" fill="#1e293b" />
                <rect x="30" y="30" width="10" height="10" fill="#10b981" />
                <rect x="45" y="30" width="10" height="10" fill="#f59e0b" />
                <rect x="60" y="30" width="10" height="10" fill="#ef4444" />
                <rect x="30" y="45" width="10" height="10" fill="#8b5cf6" />
                <rect x="45" y="45" width="10" height="10" fill="#06b6d4" />
                <rect x="60" y="45" width="10" height="10" fill="#f97316" />
                <circle cx="40" cy="65" r="4" fill="#fbbf24" />
                <circle cx="60" cy="65" r="4" fill="#ec4899" />
              </svg>

              <h3 className="text-2xl font-bold mb-4 text-green-400">Brettspiele</h3>
              <p className="text-gray-300 mb-6">
                Genieße klassische und moderne Brettspiele in einer gemütlichen sozialen Umgebung,
                perfekt für Gruppen.
              </p>
            </div>
          </div>
        </div>
      </section>

      <section id="tcg-games" className="py-20 bg-slate-700">
        <div className={containerClass}>
          <div className="text-center mb-16 md:mb-10">
            <h2 className="text-3xl md:text-4xl font-bold mb-4 text-purple-400">
              Unsere TCG Auswahl
            </h2>
            <p className="text-xl text-gray-300">
              Entdecke eine große Vielfalt an Trading Card Games in unserem Store
            </p>
          </div>

          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-7 gap-6">
            <div className={`${cardbackCardClass} hover:shadow-yellow-400/20`}>
              <img
                src={`${baseUrl}images/cardback/one-piece.png`}
                alt="One Piece Card Game"
                className="w-40 h-20 mx-auto object-contain"
              />
            </div>
            <div className={`${cardbackCardClass} hover:shadow-red-400/20`}>
              <img
                src={`${baseUrl}images/cardback/yugioh.jpg`}
                alt="Yu-Gi-Oh!"
                className="w-40 h-20 mx-auto object-contain"
              />
            </div>
            <div className={`${cardbackCardClass} hover:shadow-blue-400/20`}>
              <img
                src={`${baseUrl}images/cardback/magic.png`}
                alt="Magic the Gathering"
                className="w-40 h-20 mx-auto object-contain"
              />
            </div>
            <div className={`${cardbackCardClass} hover:shadow-green-400/20`}>
              <img
                src={`${baseUrl}images/cardback/pokemon.png`}
                alt="Pokémon"
                className="w-40 h-20 mx-auto object-contain"
              />
            </div>
            <div className={`${cardbackCardClass} hover:shadow-orange-400/20`}>
              <img
                src={`${baseUrl}images/cardback/gundam.png`}
                alt="Gundam Card Game"
                className="w-40 h-20 mx-auto object-contain"
              />
            </div>
            <div className={`${cardbackCardClass} hover:shadow-pink-400/20`}>
              <img
                src={`${baseUrl}images/cardback/lorcana.jpg`}
                alt="Lorcana"
                className="w-40 h-20 mx-auto object-contain"
              />
            </div>
            <div
              className={`col-span-full lg:col-span-1 ${cardbackCardClass} hover:shadow-black/20`}
            >
              <svg
                className="w-12 h-12 mx-auto mb-3 text-black"
                viewBox="0 0 100 100"
                fill="currentColor"
              >
                <rect x="25" y="20" width="50" height="60" rx="5" />
                <rect x="30" y="25" width="40" height="25" rx="3" fill="white" />
                <text x="50" y="46" textAnchor="middle" fontSize="30">
                  +
                </text>
                <rect x="32" y="55" width="36" height="2" rx="1" fill="white" />
                <rect x="32" y="60" width="32" height="2" rx="1" fill="white" />
                <rect x="32" y="65" width="20" height="2" rx="1" fill="white" />
              </svg>

              <h4 className="text-sm font-bold text-black">Und viele mehr!</h4>
            </div>
          </div>

          <div className="text-center mt-12 flex flex-wrap justify-center gap-4 font-semibold">
            <div className={`${badgeClass} text-purple-400`}>Turniere &amp; Events</div>
            <div className={`${badgeClass} text-cyan-400`}>Booster Packs</div>
            <div className={`${badgeClass} text-green-400`}>Einzelkarten</div>
            <div className={`${badgeClass} text-yellow-400`}>Zubehör</div>
          </div>
        </div>
      </section>

      <section id="wochenplan" className="py-20 bg-slate-800">
        <div className={containerClass}>
          <div className="text-center mb-16 md:mb-10">
            <h2 className="text-3xl md:text-4xl font-bold mb-4 text-purple-400">Wochenplan</h2>
            <p className="text-xl text-gray-300">Unsere Öffnungszeiten und geplanten Events</p>
          </div>

          <div className="overflow-x-auto">
            <table className="w-full rounded-lg overflow-hidden block md:table">
              <thead className="bg-slate-900 hidden md:table-header-group">
                <tr>
                  <th className="px-6 py-4"></th>
                  <th className="px-6 py-4"></th>
                  <th className="px-6 py-4 text-left text-sm font-semibold text-purple-400">
                    Geplantes Event
                  </th>
                </tr>
              </thead>

              <tbody className="md:divide-y divide-slate-900 block md:table-row-group">
                <tr className={`${tableRowBase} mb-0 rounded-t-lg`}>
                  <td className={`p-6 md:py-4 ${dayCellClass}`} rowSpan={3}>
                    Montag
                  </td>
                  <td className={timeCellClass}>18:30 Uhr</td>
                  <td className={activityCellClass}>Pokémon</td>
                </tr>
                <tr className={`${tableRowBase} mb-0`}>
                  <td className={timeCellClass}>18:30 Uhr</td>
                  <td className={activityCellClass}>Digimon</td>
                </tr>
                <tr className={`${tableRowBase} mb-4 rounded-b-lg`}>
                  <td className={timeCellClass}>20:00 Uhr</td>
                  <td className={activityCellClass}>Flesh & Blood</td>
                </tr>
                <tr className={`${tableRowBase} mb-4 rounded-lg border border-slate-900`}>
                  <td className={dayCellSingleClass}>Dienstag</td>
                  <td className={timeCellClass}>18:30 Uhr</td>
                  <td className={activityCellTopClass}>One Piece</td>
                </tr>
                <tr className={`${tableRowBase} mb-4 rounded-lg border border-slate-900`}>
                  <td className={dayCellSingleClass} rowSpan={2}>
                    Mittwoch
                  </td>
                  <td className={timeCellClass}>18:30 Uhr</td>
                  <td className={activityCellTopClass}>Yu-Gi-Oh! Advanced Constructed</td>
                </tr>
                <tr className={`${tableRowBase} mb-4 rounded-lg border border-slate-900`}>
                  <td className={timeCellClass}>18:30 Uhr</td>
                  <td className={activityCellTopClass}>Tekken Locals</td>
                </tr>
                <tr className={`${tableRowBase} mb-0 rounded-t-lg`}>
                  <td className={`p-6 md:py-4 ${dayCellClass}`} rowSpan={3}>
                    Donnerstag
                  </td>
                  <td className={timeCellClass}>18:30 Uhr</td>
                  <td className={activityCellClass}>Pokémon</td>
                </tr>
                <tr className={`${tableRowBase} mb-4 rounded-b-lg`}>
                  <td className={timeCellClass}>18:30 Uhr</td>
                  <td className={activityCellClass}>Gundam</td>
                </tr>
                <tr className={`${tableRowBase} mb-4 rounded-b-lg`}>
                  <td className={timeCellClass}>18:30 Uhr</td>
                  <td className={activityCellClass}>Super Smash Bros. Locals</td>
                </tr>
                <tr className={`${tableRowBase} mb-0 rounded-t-lg`}>
                  <td className={`p-6 md:py-4 ${dayCellClass}`} rowSpan={2}>
                    Freitag
                  </td>
                  <td className={timeCellClass}>18:30 Uhr</td>
                  <td className={activityCellClass}>One Piece</td>
                </tr>
                <tr className={`${tableRowBase} mb-4 rounded-b-lg`}>
                  <td className={timeCellClass}>18:30 Uhr</td>
                  <td className={activityCellClass}>Yu-Gi-Oh! Time Wizard 2005</td>
                </tr>
                <tr className={`${tableRowBase} mb-0 rounded-t-lg`}>
                  <td className={`p-6 md:py-4 ${dayCellClass}`} rowSpan={3}>
                    Samstag
                  </td>
                  <td className={timeCellClass}>16:30 Uhr</td>
                  <td className={activityCellClass}>Yu-Gi-Oh! Advanced Constructed</td>
                </tr>
                <tr className={`${tableRowBase} mb-0`}>
                  <td className={timeCellClass}>16:30 Uhr</td>
                  <td className={activityCellClass}>Yu-Gi-Oh! Time Wizard 2010</td>
                </tr>
                <tr className={`${tableRowBase} mb-4 rounded-b-lg`}>
                  <td className={timeCellClass}>18:00 Uhr</td>
                  <td className={activityCellClass}>Schach (neu!)</td>
                </tr>
                <tr className={`${tableRowBase} mb-0 rounded-t-lg`}>
                  <td className={`p-6 md:py-4 ${dayCellClass}`} rowSpan={2}>
                    Sonntag
                  </td>
                  <td className={timeCellClass}>16:30 Uhr</td>
                  <td className={activityCellClass}>Yu-Gi-Oh! Genesys</td>
                </tr>
                <tr className={`${tableRowBase} mb-4 rounded-b-lg`}>
                  <td className={timeCellClass}>16:30 Uhr</td>
                  <td className={activityCellClass}>Magic: The Gathering</td>
                </tr>
              </tbody>
            </table>
          </div>
        </div>
      </section>

      <section id="ueber-uns" className="bg-slate-900">
        <div className={`${containerClass} grid grid-cols-1 lg:grid-cols-2 gap-12 items-center`}>
          <div className="pt-20 lg:py-20 px-4 sm:px-6 lg:px-8">
            <h2 className="text-3xl md:text-4xl font-bold mb-6 text-cyan-400">
              Über unseren Store
            </h2>
            <p className="text-lg text-gray-300 mb-6">
              Wir sind leidenschaftlich dabei, das ultimative Gaming-Erlebnis für Enthusiasten aller
              Art zu schaffen. Unsere Lounge bietet einen einladenden Raum, in dem Gamer sich
              vernetzen, konkurrieren und gemeinsam Spaß haben können.
            </p>
            <p className="text-lg text-gray-300 mb-8">
              Von kompetitiven Turnieren bis hin zu entspannten Gaming-Sessions bieten wir für jeden
              etwas in unserer community-orientierten Umgebung.
            </p>

            <ul className="flex flex-wrap gap-4 font-semibold">
              <li className={`${badgeClass} text-cyan-400`}>TCG-Karten</li>
              <li className={`${badgeClass} text-purple-400`}>Videospiele</li>
              <li className={`${badgeClass} text-green-400`}>Brettspiele</li>
            </ul>
          </div>

          <img
            src={`${baseUrl}images/about-us.jpg`}
            aria-hidden="true"
            className="w-full h-full object-cover lg:[clip-path:polygon(0%_0%,100%_0%,100%_100%,15%_100%)]"
            alt=""
          />
        </div>
      </section>

      <section id="tickets" className="bg-slate-800">
        <div
          className={`${containerClass} grid grid-cols-1 lg:grid-cols-2 gap-12 items-center py-20`}
        >
          <div className="px-4 sm:px-6 lg:px-8">
            <h2 className="text-3xl md:text-4xl font-bold mb-6 text-purple-400">Tickets</h2>
            <div className="text-lg text-gray-300 space-y-4">
              <h3 className="text-2xl font-bold text-cyan-400">
                🎮 3vs3 OP16 CASE TURNIER
              </h3>
              <ul className="space-y-2">
                <li>📅 Datum: 04.06.2026, Donnerstag (Feiertag)</li>
                <li>🕦 Turnierbeginn: 11:30 Uhr (Einlass ab 10:30 Uhr)</li>
                <li>📍 1A Gamer House, Fichtelbachstr. 3, 86153 Augsburg</li>
                <li>💶 Startgebühr: 110€ pro Team (inkl. 1x TP + 2x OP-Booster p.P.)</li>
                <li>🎮 Format: Constructed | 3vs3 Ruling | TopCut (Top4)</li>
                <li>👥 Max. 30 Teams</li>
              </ul>
              <div>
                <p className="font-semibold text-white mt-2">🔥 Preispool</p>
                <p>Pro 2 Teams → 1 OP16-Display (max. 12 Displays)</p>
                <p className="text-sm text-gray-400 mt-1">Beispiel bei 24 Teams: 1. Platz 5 · 2. Platz 3 · 3.&4. Platz je 2 Displays</p>
              </div>
              <p>🎁 Ab 10 Teams: 2 zusätzliche Preisverlosungen während des Events!</p>
              <p className="text-yellow-400 font-semibold">⚠️ Teamname bitte ins Feld „vollständigen Namen" einfügen!</p>
            </div>
          </div>

          <div className="flex justify-center sm:px-6 lg:px-8">
            <iframe
              height="750"
              width="500"
              src="https://shop.weeztix.com/c6150f16-c658-11f0-a9cb-7e126431635e"
              className="w-full max-w-[500px] h-[900px] md:h-[750px] rounded-3xl overflow-hidden"
            ></iframe>
          </div>
        </div>
      </section>

      <footer className="bg-slate-800 border-t border-slate-700">
        <div className={`${containerClass} py-12`}>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            <div>
              <h3 className={footerHeadingClass}>1A Gamer Lounge</h3>
              <p className="text-gray-400">Dein ultimatives Ziel für Gaming-Exzellenz.</p>
            </div>
            <div>
              <h3 className={footerHeadingClass}>Social Media</h3>
              <a
                href="https://instagram.com/augsburg.gamerlounge"
                target="_blank"
                rel="noopener noreferrer"
                className={`inline-flex items-center ${footerLinkClass}`}
              >
                <svg className="w-5 h-5 mr-2" fill="currentColor" viewBox="0 0 24 24">
                  <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.949-.073zm0 5.838c-3.403 0-6.162 2.759-6.162 6.162s2.759 6.163 6.162 6.163 6.162-2.759 6.162-6.163c0-3.403-2.759-6.162-6.162-6.162zm0 10.162c-2.209 0-4-1.79-4-4 0-2.209 1.791-4 4-4s4 1.791 4 4c0 2.21-1.791 4-4 4zm6.406-11.845c-.796 0-1.441.645-1.441 1.44s.645 1.44 1.441 1.44c.795 0 1.439-.645 1.439-1.44s-.644-1.44-1.439-1.44z" />
                </svg>
                @augsburg.gamerlounge
              </a>
            </div>
            <div>
              <h3 className={footerHeadingClass}>Rechtliches</h3>
              <ul className="space-y-2">
                <li>
                  <a href={`${baseUrl}datenschutz/`} className={footerLinkClass}>
                    Datenschutz
                  </a>
                </li>
                <li>
                  <a href={`${baseUrl}impressum/`} className={footerLinkClass}>
                    Impressum
                  </a>
                </li>
              </ul>
            </div>
          </div>

          <p className="border-t border-slate-700 mt-8 pt-6 text-center text-gray-400">
            Play. Compete. Chill.
          </p>
        </div>
      </footer>
    </>
  );
}

const container = document.querySelector("#app")!;
const root = createRoot(container);
root.render(<App />);
